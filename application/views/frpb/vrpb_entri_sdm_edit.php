					...
					if (sizeof($rsL3)>0){
						...
						foreach ($rsL3 as $row3){	//DISPLAY LEVEL 3
							...
							if (sizeof($rsL4)>0){
								...
								//status=edit => get nominal dari table rpb_sdm_detil
							...
							if (sizeof($rsL4)>0){
								...
								foreach ($rsL4 as $row4){	//DISPLAY LEVEL 4
									$rsVal=$this->db->query("select * from rpb_sdm_detil where id_header=".$rowheader->id_header." and id_item=".$row4->id)->row();
									echo "<tr>";
									echo "<td>".$i.".".$j.".".$k."</td>";
...
$('#btvalidasi').click(function(){	
	...
	$().showMessage('Sedang diproses.. Harap tunggu..');
		$.ajax({
			type: 'POST',
			url: '<?php echo base_url('rpb/approve_sdm_form');?>',
			...
			success: function(msg) {
				...
				if(msg.status =='success'){
					$().showMessage('Data RPB BAGIAN SDM DAN UMUM berhasil diapprove.','success',2000);
