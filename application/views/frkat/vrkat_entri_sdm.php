...
<?php echo form_open('rkat/save_sdm_entri',array('class'=>'form-horizontal','id'=>'myform'));?>
						...
						foreach ($rsL3 as $row3){	//DISPLAY LEVEL 3
							...
							if (sizeof($rsL4)>0){
								...
								for ($x=1; $x<=sizeof($arrBulan);$x++){
									echo "<td><input type='hidden' name='idrkat_".$rowx."' id='idrkat_".$rowx."' value='".$row3->id."'>".form_input(array('name'=>'txtnom_'.$rowx.'_'.$x,'id'=>'txtnom_'.$rowx.'_'.$x,'class'=>'form-control' ,'onkeypress'=>"return numericVal(this,event)", 'value'=>0,  'style'=> 'width:100px',))."</td>";									
								}
								$rowx++;
							}
							echo "</tr>";
							...
							if (sizeof($rsL4)>0){
								...
								foreach ($rsL4 as $row4){	//DISPLAY LEVEL 4
									...
									//item rkat
									for ($x=1; $x<=sizeof($arrBulan);$x++){
										echo "<td><input type='hidden' name='idrkat_".$rowx."' id='idrkat_".$rowx."' value='".$row4->id."'> ".form_input(array('name'=>'txtnom_'.$rowx.'_'.$x,'id'=>'txtnom_'.$rowx.'_'.$x,'class'=>'form-control','onkeypress'=>"return numericVal(this,event)",  'style'=> 'width:100px', 'value'=>0))."</td>";										
									}
									echo "</tr>";
									$l++;
									$rowx++;
		...
		$.ajax({
			...
			success: function(msg) {
				...
				if(msg.status =='success'){
					$().showMessage('Data RKAT Bagian Sdm Dan Umum berhasil disimpan.','success',2000);
