...
<div class="row" style="text-align:center">
	<div class="col-md-12">
		<div class="form-group">
				...
				<label class="control-label">RKAT PENGHIMPUNAN TAHUN <?php echo $tahun?></label>
...
<?	$strL1="select * from mst_item_rkat where level=1 and kelompok='$kelompok' and isactive=1 order by id";
	...
	echo '<tr>';
	for ($i=1; $i<=sizeof($arrBulan);$i++){
		echo "<th>Badan</th><th>Orang</th><th>Nominal(Rp)</th>";
	}
	echo '</tr>';
	echo '</thead>';
	echo '<tbody>';
					...
					if (sizeof($rsL3)>0){
						...
						foreach ($rsL3 as $row3){	//DISPLAY LEVEL 3
							...
							if (sizeof($rsL4)>0){
								...
								$rsVal=$this->db->query("select * from rkat_php_detil where id_header=".$rowheader->id_header." and id_item=".$row3->id)->row();
								for ($x=1; $x<=sizeof($arrBulan);$x++){
									$bdn=$x."_bdn";
									$org=$x."_org";
									$nom=$x."_nom";
									echo "<td><input type='hidden' name='idrkat_".$rowx."' id='idrkat_".$rowx."' value='".$row3->id."'>".form_input(array('name'=>'txtbdn_'.$rowx.'_'.$x,'id'=>'txtbdn_'.$rowx.'_'.$x,'class'=>'form-control', 'onkeypress'=>"return numericVal(this,event)", 'value'=> (sizeof($rsVal)>0? $rsVal->$bdn:0 )  ))."</td>";
									...
									echo "<td>".form_input(array('name'=>'txtnom_'.$rowx.'_'.$x,'id'=>'txtnom_'.$rowx.'_'.$x,'class'=>'form-control' ,'onkeypress'=>"return numericVal(this,event)", 'value'=> (sizeof($rsVal)>0? $rsVal->$nom:0 )  ))."</td>";									
								}
								$rowx++;
							}
							echo "</tr>";
							...
							if (sizeof($rsL4)>0){
								...
								foreach ($rsL4 as $row4){	//DISPLAY LEVEL 4
									$rsVal=$this->db->query("select * from rkat_php_detil where id_header=".$rowheader->id_header." and id_item=".$row4->id)->row();
									echo "<tr>";
									echo "<td>".$i.".".$j.".".$k."</td>";
									...
									for ($x=1; $x<=sizeof($arrBulan);$x++){
										...
										$org=$x."_org";
										$nom=$x."_nom";
										echo "<td><input type='hidden' name='idrkat_".$rowx."' id='idrkat_".$rowx."' value='".$row4->id."'> ".form_input(array('name'=>'txtbdn_'.$rowx.'_'.$x,'id'=>'txtbdn_'.$rowx.'_'.$x,'class'=>'form-control','onkeypress'=>"return numericVal(this,event)", 'value'=> (sizeof($rsVal)>0? $rsVal->$bdn : 0 )  ))."</td>";
										echo "<td>".form_input(array('name'=>'txtorg_'.$rowx.'_'.$x,'id'=>'txtorg_'.$rowx.'_'.$x,'class'=>'form-control','onkeypress'=>"return numericVal(this,event)", 'value'=> (sizeof($rsVal)>0? $rsVal->$org : 0 )  ))."</td>";
										echo "<td>".form_input(array('name'=>'txtnom_'.$rowx.'_'.$x,'id'=>'txtnom_'.$rowx.'_'.$x,'class'=>'form-control','onkeypress'=>"return numericVal(this,event)", 'value'=> (sizeof($rsVal)>0? $rsVal->$nom : 0 )  ))."</td>";										
									}
									echo "</tr>";
...
$('#btvalidasi').click(function(){	
	$().showMessage('Sedang diproses.. Harap tunggu..');	
		$.ajax({
			type: 'POST',
			url: '<?php echo base_url('rkat/validasi_php_form');?>',
			...
			success: function(msg) {
				...
				if(msg.status =='success'){
					$().showMessage('Data RKAT Penghimpunan berhasil divalidasi.','success',2000);
...
$('#btsimpan').click(function(){		
		...
		$.ajax({
			type: 'POST',
			url: '<?php echo base_url('rkat/save_php_entri');?>',
			data: form_data,				
			dataType: 'json',
			success: function(msg) {
				// $("#errorHandler").html('&nbsp;').hide();
