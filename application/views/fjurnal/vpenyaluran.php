   ...
   <div class="panel panel-default card-view">
		...
		<div class="pull-left">
			<h6 class="panel-title txt-dark"><label id="lbltitle">Jurnal Pengeluaran Penyaluran Program</label></h6>
		</div>
...
<div class="row">
	<div class="col-md-12 form-inline"  >
		<div class="form-group col-md-4">
			<label class="col-sm-6"    >Pilih Tanggal Mulai </label>
		...
		<div class="form-group col-md-4">
			<label class="col-sm-4"    >s.d </label>
			...
			<div id="bttgl1" class="input-group-addon">  <span class="fa fa-calendar" id="bttgl2"></span>  </div>
			</div><!-- /.input group -->        
		</div><!-- /.form group -->
...
function display(tgl1, tgl2,  pilihan){
		...
		var myurl='';
		/*if (pilihan=='New')	{
			myurl='<?php echo base_url('pendapatanzis/generateData');?>';
		}else{
			myurl='<?php echo base_url('pendapatanzis/generateDataPosted');?>';
		}*/
		myurl='<?php echo base_url('penyaluran/generateData');?>';
		$.ajax({
			type: 'POST',
			url: myurl,
...
function updateForm(){
		...
		$.ajax({
			type: 'POST',
			url: '<?php echo base_url('pendapatanzis/updateJurnal');?>',
			data: form_data,				
			dataType: 'json',
			success: function(msg) {
	...
	function simpanForm(){
		...
		$.ajax({
			type: 'POST',
			url: '<?php echo base_url('penyaluran/saveJurnal');?>',
			data: form_data,				
			dataType: 'json',
			success: function(msg) {
				 $("#errorHandler").html('&nbsp;').hide();
