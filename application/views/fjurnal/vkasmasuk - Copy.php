...
<div class="panel panel-default card-view"><div class="panel-heading"><label id="lbltitle">Tambah Transaksi Kas Masuk</label></div>
...
<div class="alert alert-success alert-dismissable">
    ...
    <b>Informasi : </b> Checkbox/Opsi '<b>Anggaran dari pusat</b>' wajib dicentang jika transaksi jurnal adalah penerimaan subsidi dari pusat ke cabang. Karena transaksi ini akan masuk ke Laporan Realisasi Anggaran (sebagai penanda di Laporan cabang 'Hutang Afiliasi dari Pusat')
</div>
<?	} ?>
<div class="row">
	<div class="col-md-8">
...
<div class="row">
	<div class="col-md-8">
		<div class="form-group"><label class="col-sm-4 control-label">Opsi</label>
			<div class="checkbox col-sm-8"><label class="control-label"><input type="checkbox" name="ckOpsi" id="ckOpsi" >&nbsp;&nbsp;Anggaran dari Pusat</label></div>
		</div>
	</div>
</div>	
	<?
...
<script>
    $(document).ready(function() {
        $('#dataTables').dataTable({
			...
			"sAjaxSource": "<?php echo base_url('kasmasuk/json_data');?>"
		});
	...
	$('#btcreate').click(function(){
		...
		$('#state').val('add');
		$('#lbltitle').text('Tambah Transaksi Jurnal Pendapatan Kas & Bank');
	...
	$('#btsimpankel').click(function(){		
		...
		$.ajax({
			type: 'POST',
			url: '<?php echo base_url('kasmasuk/savekasmasuk');?>',
			data: form_data,				
			dataType: 'json',
	...
	function delkasmasuk(idx){
			...
			if (pilih==true) {
					$.ajax({
					type	: "POST",
					url		: "<?php echo base_url('kasmasuk/delkasmasuk');?>",
					data	: "idx="+idx,
					timeout	: 3000,  
	...
	function editkasmasuk(obj){
		...
		$('#lbltitle').text('Edit Data Jurnal Pendapatan Kas & Bank');
		$.ajax({
			type: 'POST',
			url: '<?php echo base_url('kasmasuk/getkasmasuk');?>',
			data: {
				id:id
			},
			dataType: 'json',
