...
<?	$i=1;
	...
	foreach ($query as $level2){
			...
			foreach ($resList4 as $level4){
				...
				echo "<tr><td>&nbsp;</td><td>&nbsp;</td><td>".strtoupper($level4->nama)."</td>";
							if (substr($level4->idacc,0,2)=='3.' ){
								...
								//$strNya=$this->report_model->getLabaRugiValueL4Str($level4->idacc,$level4->id_cab, $bln, $thn);
							}else{
							//get neraca value
		...
		echo "<tr><td>&nbsp;</td><td colspan=2><b><i>TOTAL&nbsp;".strtoupper($level2->nama)."</i></b></td><td align=right><b>Rp. ".($jumlahL3<0?"(".number_format(($jumlahL2*-1),2,',','.').")":number_format($jumlahL2,2,',','.'))."</b></td><td>&nbsp;</td></tr>";
		echo "<tr><td colspan=5>&nbsp;</td></tr>";
		$i++;	
...
<div class="row" style="text-align:center">
	<div class="col-md-12">	<input type="hidden" name="param" id="param" value="<?=$param?>">
		...
		<a href="<?=base_url('rptPenjNeracaKonsolidasi/result/'.$param)?>" class="btn btn-success">Cetak/Download</a><br>		
	</div>
</div>	
<?}?>
<script>
$('#btToExcel').click(function(){		
		...
		$.ajax({
			type: 'POST',
			url: '<?php echo base_url('rptPenjNeracaKonsolidasi/toExcel');?>',
