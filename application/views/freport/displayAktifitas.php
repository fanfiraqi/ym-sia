...
<?	$str="select * from ak_rekeningperkiraan where idparent='4.1'  order by  idacc";	
	$resList= $this->db->query($str)->result();
	$jmlIncome=0;
	if (sizeof ($resList)<=0){
		echo "<tr><td colspan=5>Rekening Perkiraan Pendapatan ZIS Tidak ada atau bukan data Cabang Anda <br>$str</td></tr>";
	}else{
		...
		foreach ($resList as $detil){
			...
			$i=1;
			$jmlIncome=0;
			$str2="select * from ak_rekeningperkiraan where idparent='".$detil->idacc."' and id_cab=".$wilFilter."  order by nama";
			$resList2= $this->db->query($str2)->result();
			//echo "<tr><td colspan=7>$str2</td></tr>";
			foreach ($resList2 as $detil2){
				$str3="select * from ak_rekeningperkiraan where idparent='".$detil2->idacc."' and id_cab=".$wilFilter."  order by nama";
				$resList3= $this->db->query($str3)->result();
				//echo "<tr><td colspan=7>$str3</td></tr>";
					foreach ($resList3 as $detil3){
						$str4="select * from ak_rekeningperkiraan where idparent='".$detil2->idacc."' and id_cab=".$wilFilter."  order by nama";
						$resList4= $this->db->query($str4)->result();
						foreach ($resList4 as $detil4){
						$rsJumlahDebet=$this->db->query("select ifnull(sum(jumlah),0) jml from ak_jurnal where idperkdebet ='".$detil4->idacc."' and bulan='$bln' and tahun='$thn' and status_validasi=1")->row();
						$rsJumlahKredit=$this->db->query("select ifnull(sum(jumlah),0) jml from ak_jurnal where idperkkredit ='".$detil4->idacc."' and bulan='$bln' and tahun='$thn' and status_validasi=1")->row();
						$jumlahP=$rsJumlahKredit->jml - $rsJumlahDebet->jml;
						//if ($jumlahP <>0){
							echo "<tr>";
							echo "<td></td>";
							echo "<td>$i</td>";
							echo "<td colspan=2>".$detil4->nama."</td>";
...
<?	$str="select * from ak_rekeningperkiraan where idparent='8-0000'  order by idacc ASC";	
	...
	$jmlLain=0;
		foreach ($resListLain as $detilLain){			
			...
			echo "<td colspan=3></td></tr>";
			$strLain2="select * from ak_rekeningperkiraan where idparent='".$detilLain->idacc."' and upper(nama) not like '%PENYUSUTAN%' and id_cab=".$wilFilter." order by  idacc";
...
<div class="row" style="text-align:center">
	<div class="col-md-12">	<input type="hidden" name="param" id="param" value="<?=$param?>">
		...
		<a href="<?=base_url('rptAktifitas/result/'.$param)?>" class="btn btn-success">Cetak/Download</a><br>		
	</div>
</div>	
<?}?>
<script>
$('#btToExcel').click(function(){		
		...
		$.ajax({
			type: 'POST',
			url: '<?php echo base_url('rptAktifitas/toExcel');?>',
