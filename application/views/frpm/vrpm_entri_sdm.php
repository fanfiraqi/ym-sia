					...
					if (sizeof($rsL3)>0){
						...
						foreach ($rsL3 as $row3){	//DISPLAY LEVEL 3
							...
							if (sizeof($rsL4)>0){
								...
								//get nominal dari tabel entri rpb approved
								$strRpb="SELECT * FROM rpb_sdm_detil WHERE id_header= (SELECT DISTINCT id_header FROM rpb_sdm_header WHERE id_cabang=".$id_cabang." AND tahun='".$tahun."' and bulan='$bulan') and id_item=".$row3->id;
								...
								//get nominal dari tabel entri rkat
									$str="SELECT * FROM rkat_sdm_detil WHERE id_header= (SELECT DISTINCT id_header FROM rkat_sdm_header WHERE id_cabang=".$id_cabang." AND tahun='".$tahun."') and id_item=".$row3->id;
									$rsVal=$this->db->query($str)->row();
									$intBln=intval($bulan);	//kolom value bulan									
									$org=$intBln."_org";	//unit
									$jml=$intBln."_jml";	// harga dari rkat
									$c_unit=$rsVal->$org;
									$c_satuan=round(($rsVal->$jml/$c_unit),2);
									$c_jumlah=$rsVal->$jml;
									$a_unit=$c_unit;
									$a_satuan=$c_satuan;
									$a_jumlah=$c_jumlah;
								...
								echo '<td><input type="text" name="txtpekan1_'.$rowx.'" id="txtpekan1_'.$rowx.'" class="form-control" onkeypress="return numericVal(this,event)" value="0" style="width:100px" ></td>';
								echo '<td><input type="text" name="txtpekan2_'.$rowx.'" id="txtpekan2_'.$rowx.'" class="form-control" onkeypress="return numericVal(this,event)" value="0" style="width:100px" ></td>';
								echo '<td><input type="text" name="txtpekan3_'.$rowx.'" id="txtpekan3_'.$rowx.'" class="form-control" onkeypress="return numericVal(this,event)" value="0" style="width:100px" ></td>';
								echo '<td><input type="text" name="txtpekan4_'.$rowx.'" id="txtpekan4_'.$rowx.'" class="form-control" onkeypress="return numericVal(this,event)" value="0" style="width:100px" ></td>';
							...
							if (sizeof($rsL4)>0){
								...
								foreach ($rsL4 as $row4){	//DISPLAY LEVEL 4
									...
									//item rkat
									//get nominal dari tabel entri rpb approved
									$strRpb="SELECT * FROM rpb_sdm_detil WHERE id_header= (SELECT DISTINCT id_header FROM rpb_sdm_header WHERE id_cabang=".$id_cabang." AND tahun='".$tahun."' and bulan='$bulan') and id_item=".$row4->id;
									$rsValRpb=$this->db->query($strRpb)->row();
									if (sizeof($rsValRpb)>0){
										$c_status=$rsValRpb->status;
										/*$c_unit=$rsValRpb->unit_cab;
									...
									//get nominal dari tabel entri rkat
										$str="SELECT * FROM rkat_sdm_detil WHERE id_header= (SELECT DISTINCT id_header FROM rkat_sdm_header WHERE id_cabang=".$id_cabang." AND tahun='".$tahun."') and id_item=".$row4->id;
										$rsVal=$this->db->query($str)->row();
										$intBln=intval($bulan);	//kolom value bulan									
										$org=$intBln."_org";	//unit
										$jml=$intBln."_jml";	// harga dari rkat
...
$('#btsimpan').click(function(){		
		...
		$.ajax({
			type: 'POST',
			url: '<?php echo base_url('rpm/save_rpm_sdm_entri');?>',
			data: form_data,				
			dataType: 'json',
			success: function(msg) {
				...
				if(msg.status =='success'){
					$().showMessage('Data RPM Sdm, Beban Umum Dan Adminsitrasi berhasil disimpan.','success',2000);
