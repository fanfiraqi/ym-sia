...
class rptPosisiKonsolidasi extends MY_App {
	var $branch = array();
	function __construct()
	{
		...
		$this->config->set_item('mysubmenu', 'menuLima11');
	...
	{
		//$akunKas=$this->divTreeKas($this->common_model->comboGetrptPenjNeracaKonsolidasi()->result_array(), '1-1100');
		$this->template->set('breadcrumbs','<li><a href="#">Laporan</a></li><li><a href="#">Posisi Keuangan Konsolidasi</a></li>');
		$this->template->set('pagetitle','Filter Laporan Posisi Keuangan Konsolidasi');		
		//$data['akunKas'] = $akunKas;
		$data['arrBulan'] = $this->arrBulan2;
		$data['arrThn'] = $this->getYearArr();
		$data["jenis"]="posisikonsolidasi";
		$data["action"]="rptPosisiKonsolidasi/result";
	...
	public function result($param=null){
		...
		$rsCab=$this->db->query("select KOTA from mst_cabang where ID_CABANG=".$cab_pilih)->row();
		$data['nmcabang']=$rsCab;*/
		$title='Laporan Neraca Konsolidasi '.$blnStr[$bln]." ".$thn;
		$data['title']=$title;
		...
		$namafile="neracaKons_".$thn.$bln;
		if ($display==0){
			$this->template->set('pagetitle',$title.' (View/Cetak)');		
			$this->template->load('default','freport/displayPosisiKonsolidasi',$data);
		}else{
			$html=$header;
			$html.=$this->load->view('freport/displayPosisiKonsolidasi', $data, true);
			$html.=$footer;
	...
	public function toExcel(){
		...
		$title='Laporan Posisi Keunangan Konsolidasi Bulan '.$blnStr[$bln]." ".$thn;
		$namafile="lapPosisiKeuanganKonsolidasi_".$thn.$bln;
...
$row++;
	...
	foreach ($query as $level2){
		...
		foreach ($query3 as $level3){
			...
			$objPHPExcel->getActiveSheet()->setCellValue('C'.$row, strtoupper($level3->nama))->setCellValue('E'.$row, ($jumlahL3<0?"(".number_format(($jumlahL3*-1),2,',','.').")":number_format($jumlahL3,2,',','.')) );
			$row++;
		...
		$objPHPExcel->getActiveSheet()->setCellValue('B'.$row, "TOTAL ".strtoupper($level2->nama))->getStyle('B'.$row)->getFont()->setBold(true);
