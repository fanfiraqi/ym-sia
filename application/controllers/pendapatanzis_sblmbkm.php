	...
	public function generateData(){
		...
		foreach($qryDonasi as $row){
			...
			foreach($qryBkm as $rowBkm){
				$rscab=$this->gate_db->query("select  idacc from mst_bank where id=".$rowBkm->bank_id)->row();	//name, account_number, idacc
				$rsakun=$this->db->query("select  * from ak_rekeningperkiraan where idacc='".trim($rscab->idacc)."'")->row();	//name, account_number, idacc
				$html.="<tr>";	
				if ($i==1){
				$html.="<td >".$i."</td>";
				$html.="<td >Debet</td>";	
