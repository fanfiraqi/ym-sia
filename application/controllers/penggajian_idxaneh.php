...
class penggajian extends MY_App {
	...
	public function saveJurnal(){
		if ($this->input->is_ajax_request()){
			...
			$pusat_kredit_coa=$this->input->post('pusat_kredit_coa');	
			$nobuktiref=$this->input->post('nobuktiref');	
		
			//cabang
			$txtjml_kredit=$this->input->post('txtjml_kredit');
			$cabang_debet_coa=$this->input->post('cabang_debet_coa');	
			$cabang_kredit_rk=$this->input->post('cabang_kredit_rk');
			$cabang_urai=$this->input->post('cabang_urai');
			$id_cabang=$this->input->post('id_cabang');
			$resultstr="";	

			for ($x=0; $x<$jmlrow;$x++){
					if (in_array($jenis, ['laz','tasharuf','thr_laz','thr_tasharuf' ]) && $id_cabang[$x]=="1"){
						//jurnal ke pusat
						$qzis_pusat="insert into ak_jurnal (bulan, tahun, id_cab, tanggal, idperkdebet, idperkkredit, nobuktiref, penanda,penanda_kredit, sumber_data, keterangan, jumlah, jenis, waktuentri, status_validasi)  values('$bln', '$thn', 1, '$tanggal', '".$pusat_debet_rk[$x]."',  '".$pusat_kredit_coa[$x]."',   '".$nobuktiref[$x]."',  '".addslashes($pusat_urai[$x])."','".addslashes($pusat_urai[$x])."', 'penggajian',  '".addslashes($pusat_urai[$x])."',  '".$txtjml_debet[$x]."', 'BKK', '".date('Y-m-d H:i:s')."', 0)";
						
						$this->db->query($qzis_pusat);	
						$this->db->trans_commit();

						//update status pos setoran zis=1
						$strUp="update ".(substr($jenis, 1,3)=="thr"?"thr_validasi":"gaji_validasi")." set sts_post_sia=1, tgl_post_sia='".date('Y-m-d')."', post_sia_by='".$this->session->userdata('auth')->id."' where id=".$id_validasi[$x]; //where ?
					...
					//jurnal cabang		
					$qzis="insert into ak_jurnal (bulan, tahun, id_cab, tanggal, idperkdebet, idperkkredit, nobuktiref, penanda, penanda_kredit, sumber_data, keterangan, jumlah, jenis, waktuentri, status_validasi)  values('$bln', '$thn', ".$id_cabang[$x].", '$tanggal', '".$cabang_debet_coa[$x]."',  '".$cabang_kredit_rk[$x]."',  '".$nobuktiref[$x]."',  '".addslashes($cabang_urai[$x])."', '".addslashes($cabang_urai[$x])."',  'penggajian',  '".addslashes($cabang_urai[$x])."',  '".$txtjml_kredit[$x]."', 'BNK', '".date('Y-m-d H:i:s')."', 0)";
		...
		if ($sql->num_rows<=0){
			...
			if ($jenis=='zisco_bonus'){	//zisco bonus gaji_validasi=jml id cabang 
				foreach($rsGaji as $rs){
					...
					$html.='<td >Debet<input type="hidden" name="id_cabang[]" id="id_cabang[]" class="form-control"  value="'.$idcabang.'"></td>';
					$html.='<td >'.$nm_rk_cabang.'<input type="hidden" name="pusat_debet_rk[]" id="pusat_debet_rk[]"  value="'.$rk_cabang.'"></td>';	//pusat					
					$html.='<td >'.form_dropdown('cabang_debet_coa[]',$akunBiayaGaji,$akunGajiCabang,'id="cabang_debet_coa[]" class="form-control"').'</td>';	//cabang					
					$html.='<td ><input type="text" name="txtjml_debet[]" id="txtjml_debet[]" class="form-control"   onblur="blurObj(this)" onclick="clickObj(this)"  value="'.$jmlGaji.'">
					</td>';
					...
					$html.="<td ></td>";
					$html.="<td >Kredit</td>";	
					$html.='<td >'.form_dropdown('pusat_kredit_coa[]',$akunBankGaji,'','id="pusat_kredit_coa_'.$i.'" class="form-control"').'</td>';	//Pusat
					$html.='<td >'.$nm_rk_pusat.'<input type="hidden" name="cabang_kredit_rk[]" id="cabang_kredit_rk_'.$i.'"  value="'.$rk_pusat.'"></td>';	//cabang
						
					$html.='<td><input type="text" name="txtjml_kredit[]" id="txtjml_kredit[]" class="form-control"   onblur="blurObj(this)" onclick="clickObj(this)"  value="'.$jmlGaji.'"></td>';
					$html.='<td >'.form_textarea(array('name'=>'cabang_urai[]','id'=>'cabang_urai[]','class'=>'form-control', 'value'=>$cabang_urai)).'<input type="hidden" value="'.$thn.$bln.'_'.$rs->JENIS.'_'.$rs->WILAYAH.'" name="nobuktiref[]" id="nobuktiref[]" ><input type="hidden" value="'.$rs->ID.'" name="id_validasi[]" id="id_validasi[]" ></td>';
					$html.="</tr>";
					$i++;
				}
				...
				foreach($rsGaji as $rs){
					...
					if ($rs->WILAYAH=="Pusat"){
						...
						$html.='<td >Debet<input type="hidden" name="id_cabang[]" id="id_cabang[]" class="form-control"  value="1"></td>';
						$html.='<td >Gaji Karyawan Pusat <input type="hidden" name="pusat_debet_rk[]" id="pusat_debet_rk[]"  value="'.$akunGajiPusat.'"></td>';	//pusat					
						$html.='<td ></td>';						
						$html.='<td ><input type="text" name="txtjml_debet[]" id="txtjml_debet[]" class="form-control"   onblur="blurObj(this)" onclick="clickObj(this)"  value="'.$jmlGaji.'">
						</td>';
						$html.='<td >'.form_textarea(array('name'=>'pusat_urai[]','id'=>'pusat_urai[]','class'=>'form-control', 'value'=>$pusat_urai)).'</td>';
						...
						$html.="<td ></td>";
						$html.="<td >Kredit</td>";	
						$html.='<td >'.form_dropdown('pusat_kredit_coa[]',$akunBankGaji,'','id="pusat_kredit_coa[]" class="form-control"').'</td>';	//Pusat
						$html.='<td ></td>';	
							
						$html.='<td><input type="text" name="txtjml_kredit[]" id="txtjml_kredit[]" class="form-control"   onblur="blurObj(this)" onclick="clickObj(this)"  value="'.$jmlGaji.'"></td>';
						$html.='<td >'.form_textarea(array('name'=>'cabang_urai[]','id'=>'cabang_urai[]','class'=>'form-control', 'value'=>$cabang_urai)).'<input type="hidden" value="'.$thn.$bln.'_'.$rs->JENIS.'_Pusat" name="nobuktiref[]" id="nobuktiref[]" ><input type="hidden" value="'.$rs->ID.'" name="id_validasi[]" id="id_validasi[]" ></td>';
						$html.="</tr>";
						$i++;
					...
					foreach($rsTotGaji as $rsGajiCabang){
						...
						$html.='<td >Debet<input type="hidden" name="id_cabang[]" id="id_cabang_'.$i.'" class="form-control"  value="'.$idcabang.'"></td>';
						$html.='<td >'.$nm_rk_cabang.'<input type="hidden" name="pusat_debet_rk[]" id="pusat_debet_rk_'.$i.'"  value="'.$rk_cabang.'"></td>';	//pusat					
						$html.='<td >'.form_dropdown('cabang_debet_coa[]',$akunBiayaGaji,$akunGajiCabang,'id="cabang_debet_coa_'.$i.'" class="form-control"').'</td>';	//cabang					
						$html.='<td ><input type="text" name="txtjml_debet[]" id="txtjml_debet_'.$i.'" class="form-control"   onblur="blurObj(this)" onclick="clickObj(this)"  value="'.$jmlGaji.'">
						</td>';
						$html.='<td >'.form_textarea(array('name'=>'pusat_urai[]','id'=>'pusat_urai[]','class'=>'form-control', 'value'=>$pusat_urai)).'</td>';
						$html.="</tr>";
						...
						$html.='<td><input type="text" name="txtjml_kredit[]" id="txtjml_kredit[]" class="form-control"   onblur="blurObj(this)" onclick="clickObj(this)"  value="'.$jmlGaji.'"></td>';
						$html.='<td >'.form_textarea(array('name'=>'cabang_urai[]','id'=>'cabang_urai[]','class'=>'form-control', 'value'=>$cabang_urai)).'<input type="hidden" value="'.$thn.$bln.'_'.$rs->JENIS.'_'.$rs->WILAYAH.'_'.$rsGajiCabang->id_cabang.'" name="nobuktiref[]" id="nobuktiref[]" ><input type="hidden" value="'.$rs->ID.'" name="id_validasi[]" id="id_validasi[]" ></td>';
						$html.="</tr>";
						$i++;
						}
					}
