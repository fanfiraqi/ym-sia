...
class pendapatanzis extends MY_App {
	...
	public function generateData(){
		...
		//PROSES
		$strCabang="SELECT DISTINCT  cabang_id FROM bkm WHERE ( bkm_date BETWEEN ''$tgl1'  AND '$tgl2' ' )  AND `STATUS`='VALIDATED' AND sts_post_sia=0  ORDER BY bank_id ";
		...
		foreach($qry_cabang as $rescabang){
			...
			$strBkm="SELECT DISTINCT bank_id FROM bkm WHERE ( bkm_date BETWEEN '$tgl1'  AND '$tgl2')  and `STATUS`='VALIDATED' AND sts_post_sia=0  and cabang_id=".$res_idcabang." ORDER BY bank_id ";
		...
		foreach($qryBank_cabang as $row){			
			...
			$strDonasi="SELECT j.title, j.type, j.idacc_zis, d.* FROM detail_donasi d, jenis_donasi j WHERE donasi_id IN (SELECT DISTINCT id FROM donasi WHERE bkm_id IN ( SELECT DISTINCT id FROM bkm WHERE `STATUS`='VALIDATED' AND sts_post_sia=0 AND bank_id=".$row->bank_id." AND cabang_id= ".$res_idcabang.")) AND j.id=d.jenis_donasi_id  ";
			...
			foreach($qryDonasi as $rowDonasi){
					...
					$html.='<td >'.form_dropdown('cabang_kredit_coa[]',$akunPenerimaan,$coaKredit,'id="cabang_kredit_coa[]" class="form-control"').'</td>';					
