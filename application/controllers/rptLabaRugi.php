...
class rptLabaRugi extends MY_App {
	var $branch = array();
	...
	{
		$this->template->set('pagetitle','Filter Laporan Laba/Rugi');	
		$data['arrBulan'] = $this->arrBulan2;
		$data['arrThn'] = $this->getYearArr();
		$data["jenis"]="labarugi";
		$data["action"]="rptLabaRugi/result";
	...
	public function result($param=null){
		...
		$title='Laporan Laba Rugi '.$rsCab->KOTA.' Per '.date('d').' '.$blnStr[$bln]." ".$thn;
		$data['title']=$title;
		...
		$namafile="lapLabaRugi_".$rsCab->KOTA.$thn.$bln;
		if ($display==0){
			$this->template->set('pagetitle',$title.' (View/Cetak)');		
			$this->template->load('default','freport/displayLabaRugi',$data);
		}else{
			$html=$header;
			$html.=$this->load->view('freport/displayLabaRugi', $data, true);
			$html.=$footer;
	...
	public function toExcel(){
		...
		$title='Laporan Laba Rugi '.$nmcabang->KOTA.' Bulan '.$blnStr[$bln];
		$namafile="lapLabaRugi_".$nmcabang->KOTA.$thn.$bln;
		...
		$objPHPExcel->getActiveSheet()->setTitle('Lap_LabaRugi_'.$nmcabang->KOTA.$thn.$bln);
	...
	$row=5;
		...
		$str="select * from ak_rekeningperkiraan where idparent='4-1000' and id_cab=".$cab_pilih." order by  idacc";	
	...
	foreach ($resList as $detil){
		...
		foreach ($resListL3 as $detilL3){
			...
			$row++;
			$strL4="select * from ak_rekeningperkiraan where idparent='".$detilL3->idacc."'  and id_cab=".$cab_pilih."  order by  idacc";
