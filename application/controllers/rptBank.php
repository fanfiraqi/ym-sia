...
class rptBank extends MY_App {
	...
	{	
		$size=$this->common_model->comboGetrptBank()->result_array();
		//echo sizeof($size);
		if (sizeof($size)>0){
			$akunBank=$this->divTreeKas($this->common_model->comboGetrptBank()->result_array(), '1.1.2');
		}else{
			$akunBank=array(''=>'');
		}
		$this->template->set('breadcrumbs','<li><a href="#">Laporan</a></li><li><a href="#">Bank Register</a></li>');
		$this->template->set('pagetitle','Filter Bank Register');		
		$data['akunBank'] = $akunBank;
		$data['arrBulan'] = $this->arrBulan2;
		$data['arrThn'] = $this->getYearArr();
		$data["jenis"]="bank";
		$data["action"]="rptBank/result";
	...
	public function result($param=null){
		...
		if ($param!=null){
			...
			$idacc=$this->input->post('akunBank');
			if ($sesLogin=='center'){
		...
		$idacc=($idacc==""?"1.1.2.".$cab_2d:$idacc);	//level 4

		$accName=$this->report_model->getAccName($idacc);

		$title='Daftar Bank Register periode '.$tgl1." - ".$tgl2;
		$data['title']=$title;
		...
		$namafile="bankRegister_".$tgl1."_".$tgl2;
		if ($display==0){
			$this->template->set('breadcrumbs','<li><a href="#">Laporan</a></li><li><a href="#">Bank Register</a></li>');
			$this->template->set('pagetitle',$title.' (View/Cetak)');		
			$this->template->load('default','freport/displayBank',$data);
		}else{
			$html=$header;
			$html.=$this->load->view('freport/displayBank', $data, true);
			$html.=$footer;
			//echo $html;
