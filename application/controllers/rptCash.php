...
class rptCash extends MY_App {
	...
	function __construct()
	{
		...
		$this->config->set_item('mysubmenu', 'menuLima3');
	...
	{
		$size=$this->common_model->comboGetrptCash()->result_array();
		//echo sizeof($size);
		$akunKas=array(''=>'');
		if (sizeof($size)>0){
			//$akunKas=$this->divTreeKas($this->common_model->comboGetrptCash()->result_array(), '1-1100');
			$akunKas=$this->common_model->comboGetrptCash()->result_array();
		}
		
		$this->template->set('breadcrumbs','<li><a href="#">Laporan</a></li><li><a href="#">Cash Register</a></li>');
		$this->template->set('pagetitle','Filter Cash Register');		
		$data['akunKas'] = $akunKas;
		$data['arrBulan'] = $this->arrBulan2;
		$data['arrThn'] = $this->getYearArr();
		$data["jenis"]="cashregister";
		$data["action"]="rptCash/result";
		//$data["strx"]=$strx;
	...
	public function result($param=null){
		...
		$idacc=($idacc==""?"1.1.1.".$cab_2d:$idacc);	//level 4
		...
		$title='Daftar Cash Register '.$rsCab->KOTA.' '.$tgl1." s.d ".$tgl2;
		$data['title']=$title;
		...
		$namafile="cashregister_".$rsCab->KOTA.$tgl1."_".$tgl2;
		if ($display==0){
			$this->template->set('pagetitle',$title.' (View/Cetak)');		
			$this->template->load('default','freport/displayCash',$data);
		}else{
			$html=$header;
			$html.=$this->load->view('freport/displayCash', $data, true);
			$html.=$footer;
	...
	public function toExcel(){
		...
		$title='Cash Register '.$nmcabang->KOTA.' antara '.$tgl1." s.d ".$tgl2;
		$namafile="cashRegister_".$nmcabang->KOTA."_".$tgl1."_".$tgl2;
		...
		$objPHPExcel->getActiveSheet()->setTitle('CashRegister');
