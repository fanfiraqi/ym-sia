...
class rptPenjNeracaKonsolidasi extends MY_App {
	var $branch = array();
	function __construct()
	{
		...
		$this->config->set_item('mysubmenu', 'menuTigaDuabelas');
	...
	public function index()
	{
		...
		$this->template->set('pagetitle','Filter Laporan Penjelasan Neraca Konsolidasi');		
		//$data['akunKas'] = $akunKas;
		$data['arrBulan'] = $this->arrBulan2;
		$data['arrThn'] = $this->getYearArr();
		$data["jenis"]="posisikonsolidasi";
		$data["action"]="rptPenjNeracaKonsolidasi/result";
	...
	public function result($param=null){
		...
		$data['nmcabang']=$rsCab;*/
		$title='Laporan Penjelasan Neraca Konsolidasi '.$blnStr[$bln]." ".$thn;
		$data['title']=$title;
		...
		$namafile="penjNeracaKons_".$thn.$bln;
		if ($display==0){
			$this->template->set('pagetitle',$title.' (View/Cetak)');		
			$this->template->load('default','freport/displayPenjNeracaKonsolidasi',$data);
		}else{
			$html=$header;
			$html.=$this->load->view('freport/displayPenjNeracaKonsolidasi', $data, true);
			$html.=$footer;
	...
	public function toExcel(){
		...
		$title='Laporan Penjelasan Neraca Konsolidasi Bulan '.$blnStr[$bln]." ".$thn;
		$namafile="lapPenjNeracaKonsolidasi_".$thn.$bln;
		...
		foreach ($query3 as $level3){
			...
			foreach ($resList4 as $level4){
				...
				//echo "<tr><td>&nbsp;</td><td>&nbsp;</td><td>".strtoupper($level4->nama)."</td>";
							if (substr($level4->idacc,0,3)=='3-8' ){
			...
			$objPHPExcel->getActiveSheet()->setCellValue('C'.$row, "TOTAL ". strtoupper($level3->nama))->setCellValue('E'.$row, ($jumlahL3<0?"(".number_format(($jumlahL3*-1),2,',','.').")":number_format($jumlahL3,2,',','.')) );
			$row++;
